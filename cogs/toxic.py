...
import discord
from utils import get_logging_channel, normalize_text, lastMessage
...
def detoxify(model, message):
    ...
    if prediction[0] > 0.95:
        ...
        [4]) > 0.007:  # there has got to be another way to format 1.234567e-8
        return True, prediction[0], prediction[1]
...
class Toxic(commands.Cog):
    ...
    async def toxicpredict(self, ctx, *, args):
        ...
        await ctx.send(f"{msg} `{str(toxicity)} | {str(severe)}` {lastMessage(ctx.channel, ctx.author.id)}")
